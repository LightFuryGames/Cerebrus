...
def test_process_memreport_filename_metadata_driven(mock_report_context, tmp_path):
    ...
    with patch(
        "cerebrus.tools.memreport.tool.HTML_TEMPLATE",
        "{metadata_json} {title} {report_title} {tab_buttons} {tab_contents}",
    ):
        process_memreport(
...
def test_process_memreport_embeds_metadata(mock_report_context, tmp_path):
    ...
    with patch(
        "cerebrus.tools.memreport.tool.HTML_TEMPLATE", "METADATA: {metadata_json}"
    ):
        process_memreport(
            input_file=input_file,
            output_dir=output_dir,
