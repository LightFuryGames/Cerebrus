...
def test_pyinstaller_spec_bundles_plugin_resources():
    ...
    assert 'plugin_resources_dir = cerebrus_dir / "plugins" / "resources"' in spec_text
    assert "plugin_resources_dir.rglob('*')" in spec_text
    assert "cerebrus/plugins/resources" in spec_text
