...
class MemoryStatsTab(ReportTab):
    ...
    def parse(self, line: str, context: Dict[str, Any]) -> None:
        ...
        if "PooledVirtualMemoryAllocator" in line:
            ...
            alloc_root = next((x for x in tree if x["name"] == "Pooled Virtual Memory Allocator"), None)
            if not alloc_root:
                alloc_root = {"name": "Pooled Virtual Memory Allocator", "value": "", "children": []}
            ...
            # Enforce spacing
            clean_name = re.sub(r'(\d)(MB|KB)', r'\1 \2', clean_name, flags=re.IGNORECASE)
            
            alloc_root["children"].append({"name": clean_name, "value": "", "children": []})
            return
        ...
        if "Pool[" in line:
            ...
            if alloc_root and alloc_root["children"]:
                ...
                clean_line = re.sub(r'(\d)(MB|KB)', r'\1 \2', clean_line, flags=re.IGNORECASE)
                
                alloc_root["children"][-1]["children"].append({"name": clean_line, "value": "", "children": []})
            return
        ...
        if " - STAT_" in line:
            ...
            if len(parts) >= 3: 
                    ...
                    group_node = next((x for x in cat_node["children"] if x["name"] == group), None)
                    if not group_node:
                        group_node = {"name": group, "value": "", "children": []}
                        cat_node["children"].append(group_node)
                    
                    group_node["children"].append({"name": name, "value": val_str, "children": []})
            return
