...
def _show_create_palette_dialog(state: UIState) -> None:
    ...
    with dpg.window(
        ...
        dpg.add_input_text(tag="cp_name_input", label="Palette Name", hint="e.g. My Custom Theme")
        dpg.add_combo(
            tag="cp_base_combo",
            label="Base On",
            items=base_palettes,
            default_value=tm.current_palette if tm.current_palette in base_palettes else base_palettes[0],
        )
...
def _handle_create_palette(state: UIState) -> None:
    ...
    if success_count > 0:
        log_message(state, "SUCCESS", f"Created palette '{name}' with {success_count} modes.")
            ...
            try:
                with open(dest, "r") as f:
                    ...
                    if p_name:
                        ...
                        log_message(state, "INFO", f"Switched to imported palette '{p_name}'.")
            except:
                pass
...
def _show_theme_editor(state: UIState, palette: str = None) -> None:
    ...
    def _open_color_picker_modal(label, key, initial_color, callback, button_tag):
        ...
        with dpg.window(tag=modal_tag, label=f"Edit {label}", modal=True, width=400, height=450):
            ...
            with dpg.group(tag=container, parent=tag):
                ...
                with dpg.tab_bar():
                                         ...
                                         with dpg.table_row():
                                             ...
                                             dpg.add_color_button(
                                                 ...
                                                 callback=lambda s, a, u: _open_color_picker_modal(
                                                     u[0], u[1], u[2], 
                                                     lambda s, a, k: (
                                                         tm.update_theme_color(palette, selected_mode, "colors", k, a)
                                                     ),
                                                     s # Pass button tag
                                                 ),
                    ...
                    with dpg.tab(label="Custom Colors"):
                        dpg.add_text("Application Specific Colors (Requires Restart for some)")
                        ...
                        for k in ["header", "subheader"]:
                            ...
                            with dpg.group(horizontal=True):
                                ...
                                dpg.add_color_button(
                                     tag=btn_tag,
                                     default_value=val, width=50, height=25,
                                     label=k.title(),
                                     callback=lambda s, a, u: _open_color_picker_modal(
                                         u[0], u[1], u[2],
                                         lambda s, a, k: tm.update_theme_color(palette, selected_mode, "label_colors", k, a),
                                         s
                                     ),
                        ...
                        for k in ["DEFAULT", "LOADED", "ERROR", "INFO"]:
                            ...
                            with dpg.group(horizontal=True):
                                ...
                                dpg.add_color_button(
                                     ...
                                     callback=lambda s, a, u: _open_color_picker_modal(
                                         u[0], u[1], u[2],
                                         lambda s, a, k: tm.update_theme_color(palette, selected_mode, "profile_status_colors", k, a),
                                         s
                                     ),
                                     user_data=(k, k, val)
                                )
                            ...
                            with dpg.group(horizontal=True):
                                ...
                                dpg.add_color_button(
                                     ...
                                     callback=lambda s, a, u: _open_color_picker_modal(
                                         u[0], u[1], u[2],
                                         lambda s, a, k: (
                                            tm.update_theme_color(palette, selected_mode, "log_colors", k, a),
                                            _render_log_entries(state)
                                         ),
                                         s
                                     ),
