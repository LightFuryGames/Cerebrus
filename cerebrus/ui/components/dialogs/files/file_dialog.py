...
def _handle_path_selected_generic(
    state: UIState, path_type: str, app_data: dict
) -> None:
...
def _browse_folder_native(state: UIState, purpose: str = "output") -> None:
    ...
    try:
        ...
        path_str = filedialog.askdirectory(
            title="Select Folder", initialdir=initial_dir
        )
...
def _show_html_file_selector(state: UIState, html_files: list) -> None:
    ...
    with dpg.window(**config.get_component_settings("html_viewer_dialog")):
        ...
        dpg.bind_item_theme(
            dpg.add_text("Available HTML Log Files:"),
            get_theme_manager().get_header_theme(),
        )
        dpg.add_separator()
