...
def _show_about_dialog(state: UIState) -> None:
    ...
    with dpg.window(
        ...
        dpg.add_spacer(height=10)
        dpg.add_text("Python-based Windows-only toolkit with DearPyGUI UI for managing", wrap=600)
        ...
        with dpg.group(horizontal=True):
            ...
            dpg.add_button(
                ...
                callback=lambda: _open_url("https://github.com/LightFuryGames/Cerebrus?tab=BSD-3-Clause-1-ov-file"),
                small=True
            )
            dpg.bind_item_theme(dpg.last_item(), link_theme)
            dpg.add_text(" .")
        ...
        with dpg.group(horizontal=True):
            dpg.add_spacer(width=180)
            dpg.add_button(
                label="Close", width=100, callback=lambda: dpg.delete_item("about_dialog")
            )
