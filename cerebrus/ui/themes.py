...
class ThemeManager:
    def __init__(self):
        ...
        self.palettes_dir = Path(__file__).parent / "resources" / "AppColorPalettes"
        self._discover_and_load_themes()

    def _discover_and_load_themes(self):
        """Scan the palettes directory and load all JSON themes."""
        if not self.palettes_dir.exists():
            self.palettes_dir.mkdir(parents=True, exist_ok=True)

        for json_file in self.palettes_dir.glob("*.json"):
            try:
    ...
    def _create_dpg_theme(self, palette, mode, data):
        ...
        if palette not in self.theme_color_ids: self.theme_color_ids[palette] = {}
        self.theme_color_ids[palette][mode] = {}
            ...
            with dpg.theme_component(dpg.mvAll):
                for col_name, color in colors.items():
                    if hasattr(dpg, col_name):
                        ...
                        item_id = dpg.add_theme_color(col_id, color, category=dpg.mvThemeCat_Core)
    ...
    def _initialize_aux_themes(self):
        ...
        def create_simple_theme(tag, default_color):
            ...
            with dpg.theme(tag=tag):
                with dpg.theme_component(dpg.mvAll):
                    return dpg.add_theme_color(dpg.mvThemeCol_Text, default_color, category=dpg.mvThemeCat_Core)

        self.aux_theme_ids["header"] = create_simple_theme("theme_aux_header", (120, 180, 255))
        self.aux_theme_ids["subheader"] = create_simple_theme("theme_aux_subheader", (200, 200, 200))
        
        # Profile Status Themes
        self.aux_theme_ids["profile_status"] = {}
        for status in ["DEFAULT", "LOADED", "ERROR", "INFO"]:
            self.aux_theme_ids["profile_status"][status] = create_simple_theme(f"theme_aux_profile_{status}", (255, 255, 255))

        # Log Themes
        self.aux_theme_ids["log"] = {}
        for level in ["DEBUG", "INFO", "WARNING", "ERROR", "SUCCESS"]:
            self.aux_theme_ids["log"][level] = create_simple_theme(f"theme_aux_log_{level}", (255, 255, 255))
            
        self.aux_theme_ids["help_button"] = create_simple_theme("theme_aux_help", (100, 100, 100))

        # Transparent Log Input Theme
        if dpg.does_item_exist("theme_log_input"): dpg.delete_item("theme_log_input")
    ...
    def _update_aux_themes(self):
        ...
        if "subheader" in lbl and self.aux_theme_ids["subheader"]:
            dpg.configure_item(self.aux_theme_ids["subheader"], value=tuple(lbl["subheader"]))

        # Update Profile Status
        st = data.get("profile_status_colors", {})
        for status, color in st.items():
            if status in self.aux_theme_ids["profile_status"]:
                 dpg.configure_item(self.aux_theme_ids["profile_status"][status], value=tuple(color))
        ...
        if "hyperlink" in lbl and self.aux_theme_ids["hyperlink"]:
             dpg.configure_item(self.aux_theme_ids["hyperlink"], value=tuple(lbl["hyperlink"]))
             
        if "help_button" in lbl and self.aux_theme_ids["help_button"]:
             dpg.configure_item(self.aux_theme_ids["help_button"], value=tuple(lbl["help_button"]))
    ...
    def get_header_color(self):
        ...
        # Fallback
        mode = self.current_mode
        if mode == "System": mode = self._get_system_theme()
    ...
    def get_profile_status_colors(self):
        ...
        if mode == "Light":
            return {
                "DEFAULT": (110, 60, 0), "LOADED": (0, 90, 0),
                "ERROR": (140, 0, 0), "INFO": (0, 40, 120),
            }
        else:
            return {
                "DEFAULT": (255, 210, 120), "LOADED": (15, 240, 15),
                "ERROR": (255, 120, 120), "INFO": (120, 200, 255)
            }
    ...
    def get_log_colors(self):
        ...
        if mode == "Light":
            return {
                "DEBUG": (80, 80, 90), "INFO": (0, 40, 120),
                "WARNING": (110, 60, 0), "ERROR": (140, 0, 0), "SUCCESS": (0, 90, 0),
            }
        else:
            return {
                "DEBUG": (170, 170, 170), "INFO": (120, 200, 255),
                "WARNING": (255, 210, 120), "ERROR": (255, 120, 120), "SUCCESS": (15, 240, 15),
            }
    ...
    def update_theme_color(self, palette: str, mode: str, category: str, key: str, value: tuple | list) -> None:
        """
        ...
        if mode not in self.themes[palette]:
            self.themes[palette][mode] = {"palette": palette, "mode": mode, "colors": {}}
        ...
        if category == "colors":
            ...
            if value and len(value) >= 3 and isinstance(value[0], float) and max(value) <= 1.0:
            ...
            if item_id and dpg.does_item_exist(item_id):
                ...
                is_system_match = (self.current_mode == "System" and self._get_system_theme() == mode)
    ...
    def save_theme(self, palette: str, mode: str) -> None:
        ...
        file_path = self.palettes_dir / filename
