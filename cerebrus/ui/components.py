    ...
    def check_thread():
        ...
        if is_available:
            ...
            if not silent_on_up_to_date:
                if latest_tag:
                    log_message(
                        state, "SUCCESS", f"You are up to date (Latest: {latest_tag})"
                    )
                else:
...
def _handle_generate_mem_report(state: UIState) -> None:
    ...
    if not source_dir.exists():
        log_message(
            state, "WARNING", f"MemReports source directory not found: {source_dir}"
        )
        return
    ...
    if not dest_dir.exists():
        try:
            ...
            log_message(
                state, "ERROR", f"Failed to create output directory {dest_dir}: {e}"
            )
            return
    ...
    log_message(
        state,
        "INFO",
        f"Found {len(report_files)} memreport files. Starting generation...",
    )
    ...
    for report_file in report_files:
        try:
            ...
            try:
                ...
                log_message(
                    state, "WARNING", f"Failed to delete source {report_file.name}: {e}"
                )
...
def _send_console_command_wrapper(state: UIState, command: str) -> None:
    ...
    if not client.is_package_running(state.selected_device_serial, state.package_name):
        ...
        log_message(
            state,
            "ERROR",
            f"Cannot send '{command}' if the application is not running.",
        )
        return
    ...
    try:
        ...
        if match:
            ...
            log_message(
                state,
                "WARNING",
                f"Metadata injection failed: Could not find 'Frame count' row in {html_file.name}",
            )
        ...
        log_message(
            state, "ERROR", f"Failed to inject metadata into {html_file.name}: {e}"
        )
    ...
    try:
        ...
        if folder_path:  # User selected a folder
            ...
            if path_type == "input":
                ...
                if dpg.does_item_exist("config_output_path_label"):
                    dpg.set_value(
                        "config_output_path_label", str(state.config_output_path)
                    )
                log_message(
                    state,
                    "SUCCESS",
                    f"Config output path set to: {state.config_output_path}",
                )
...
def _show_profile_dialog(state: UIState, is_edit: bool = False) -> None:
    ...
    with dpg.window(
        ...
        with dpg.table(header_row=False, policy=dpg.mvTable_SizingStretchProp):
            ...
            remote_configs = (
                state.profile_manager.current_profile.remote_configs
                if state.profile_manager.current_profile
                else {"Development": "", "Shipping": "", "Debug": ""}
            )
            ...
            with dpg.table_row():
                ...
                dpg.add_input_text(
                    tag="pd_base_url",
                    default_value=(
                        state.profile_manager.current_profile.remote_config_base_url
                        if state.profile_manager.current_profile
                        else ""
                    ),
            ...
            for env in ["Development", "Shipping", "Debug"]:
                with dpg.table_row():
                    ...
                    dpg.add_input_text(
                        tag=f"pd_url_{env}", default_value=remote_configs.get(env, "")
                    )
                    dpg.add_spacer()
            ...
            with dpg.table_row():
                ...
                dpg.add_input_text(
                    tag="pd_aws_access_key",
                    default_value=profile.aws_access_key if is_edit else "",
                    password=True,
                )
                dpg.add_spacer()
            ...
            with dpg.table_row():
                ...
                dpg.add_input_text(
                    tag="pd_aws_secret_key",
                    default_value=profile.aws_secret_key if is_edit else "",
                    password=True,
                )
                dpg.add_spacer()
            ...
            with dpg.table_row():
                ...
                dpg.add_input_text(
                    tag="pd_aws_region",
                    default_value=profile.aws_region if is_edit else "ap-south-1",
                )
                dpg.add_spacer()
            ...
            with dpg.table_row():
                ...
                dpg.add_input_text(
                    tag="pd_aws_profile",
                    default_value=profile.aws_profile if is_edit else "",
                    hint="e.g. default, work",
                )
                dpg.add_spacer()
...
def _handle_profile_save(state: UIState, is_edit: bool) -> None:
    ...
    if not is_edit or not state.profile_manager.current_profile_path:
        ...
        state.temp_profile_data = {
            ...
            "remote_configs": {
                env: dpg.get_value(f"pd_url_{env}")
                for env in ["Development", "Shipping", "Debug"]
            },
...
def _add_hyperlink(
    text: str, url: str, color: tuple[int, int, int] = (100, 150, 255)
) -> None:
...
def _show_aws_not_configured_modal(state: UIState, on_success_callback) -> None:
    ...
    with dpg.window(
        ...
        with dpg.group(horizontal=True):
            dpg.add_button(
                label="Configure Now", width=140, callback=_on_configure_click
            )
            dpg.add_button(
                label="Cancel",
                width=100,
...
def build_remote_config_sync(state: UIState) -> None:
    ...
    with dpg.group():
        ...
        dpg.add_text(
            source_msg,
            color=(150, 255, 150, 255) if "S3" in source_msg else (255, 200, 100, 255),
            bullet=True,
        )
        ...
        with dpg.table(header_row=False, policy=dpg.mvTable_SizingStretchProp):
            ...
            with dpg.table_row():
                ...
                with dpg.group():
                    ...
                    with dpg.child_window(
                        tag="config_files_list_container",
                        border=True,
                        height=250,
                        autosize_x=True,
                    ):
                ...
                with dpg.group():
                    with dpg.group(horizontal=True, horizontal_spacing=8):
                        dpg.add_text(
                            "Configs on Device (Persistent):", color=(120, 180, 255)
                        )
                        dpg.add_button(
                            label="Refresh",
                            width=100,
                        ...
                        dpg.add_button(
                            label="Delete All",
                            width=100,
                            callback=lambda: _handle_delete_all_configs_on_device(
                                state
                            ),
                        )
                    with dpg.child_window(
                        tag="device_config_files_list_container",
                        border=True,
                        height=250,
                        autosize_x=True,
                    ):
    ...
    if bucket and key:
        ...
        try:
            ...
            log_message(
                state,
                "WARNING",
                "No AWS credentials found (setup in Profile settings). Falling back to public URL request.",
            )
        except Exception as e:
...
def _show_aws_config_dialog(state: UIState) -> None:
    ...
    with dpg.window(
        ...
        dpg.add_text(
            "Configure AWS credentials for restricted S3 buckets.",
            color=(120, 180, 255),
        )
        dpg.add_spacer(height=10)
