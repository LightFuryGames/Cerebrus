...
def parse_timestamp(content: str, filename: str) -> str:
    match = re.search(r"Profile\((\d{4})(\d{2})(\d{2})_(\d{2})(\d{2})(\d{2})\)", content)
    if not match:
        match = re.search(r"Profile\((\d{4})(\d{2})(\d{2})_(\d{2})(\d{2})(\d{2})\)", filename)
    if match:
...
def extract_flawed_metadata(html_content: str) -> OrderedDict:
    match = re.search(r"\[HasHeaderRowAtEnd\],.*?(?=</pre>|\n\n|$)", html_content, re.DOTALL)
    if not match:
        return OrderedDict()
    ...
    for i in range(0, len(parts) - 1, 2):
        ...
        if key == "[HasHeaderRowAtEnd]": key = "HasHeaderRowAtEnd"
        elif key == "[platform]": key = "platform"
        elif key == "[config]": key = "config"
        elif key == "[buildversion]": key = "Build Version"
        elif key == "[engineversion]": key = "engineversion"
        elif key == "[os]": key = "OS"
        elif key == "[cpu]": key = "CPU/Device"
...
def extract_summary_metrics(html_content: str) -> dict:
    metrics = {}
    fc_match = re.search(r"<tr><td[^>]*>Frame count</td><td>(.*?)</td></tr>", html_content, re.DOTALL)
    if fc_match: metrics["Frame count"] = clean_html(fc_match.group(1))

    fps_table_match = re.search(r"FPSChart</h2>.*?<table.*?>(.*?)</table>", html_content, re.DOTALL)
    if fps_table_match:
        table_content = fps_table_match.group(1)
        header_row = re.search(r"<tr>(.*?)</tr>", table_content, re.DOTALL)
        if header_row:
            headers = [clean_html(h) for h in re.findall(r"<th[^>]*>(.*?)</th>", header_row.group(1), re.DOTALL)]
            data_row = re.search(r"<tr><td>Entire Run</td>(.*?)</tr>", table_content, re.DOTALL)
            if data_row:
                cells = [clean_html(c) for c in re.findall(r"<td[^>]*>(.*?)</td>", data_row.group(1), re.DOTALL)]
                for i, cell in enumerate(cells):
                    if i + 1 < len(headers):
                        metrics[headers[i+1]] = cell

    sd_match = re.search(r"Standard Deviation \(SD\):\s*([\d.]+)", html_content)
    if sd_match: metrics["Standard Deviation (SD)"] = float(sd_match.group(1))
    iqr_match = re.search(r"Interquartile Range \(IQR\):\s*([\d.]+)", html_content)
    if iqr_match: metrics["Interquartile Range (IQR)"] = float(iqr_match.group(1))
    
    gauge_blocks = re.findall(r'<div class="gauge-wrapper".*?>(.*?)</div>\s*</div>', html_content, re.DOTALL)
    for block in gauge_blocks:
        ...
        if title_match and value_match:
            ...
            if t == "1st Percentile": t = "1th Percentile"
            metrics[t] = float(value_match.group(1).strip())

    hitch_section = re.search(r"Hitches</h2>.*?<table.*?>(.*?)</table>", html_content, re.DOTALL)
    if hitch_section:
        table_content = hitch_section.group(1)
        headers = [h.strip() for h in re.findall(r"<th>\s*>\s*(\d+ms)", table_content)]
        rows = re.findall(r"<tr><td><b>(.*?)</b></td>(.*?)</tr>", table_content, re.DOTALL)
        for row_name, row_content in rows:
            cells = [clean_html(c) for c in re.findall(r"<td[^>]*>(.*?)</td>", row_content, re.DOTALL)]
            for i, cell in enumerate(cells):
                if i < len(headers):
...
def flatten_report(file_path: str) -> OrderedDict:
    ...
    if device_id.endswith('_1'): device_id = device_id[:-2]
    ...
    for in_k, out_k in summary_map:
        ...
        if val is not None:
            try:
                if isinstance(val, (int, float)): result[out_k] = val
                else: result[out_k] = float(str(val).replace(',', ''))
            except: result[out_k] = val

    hitch_rows = ["FrameTime", "GameThreadTime", "RenderThreadTime", "RHIThreadTime", "GPUTime"]
    ...
    result["MemoryFreeMB Min"] = float(str(summary.get("MemoryFreeMB Min", 0)).replace(',', ''))
    result["PhysicalUsedMB Max"] = float(str(summary.get("PhysicalUsedMB Max", 0)).replace(',', ''))
    result["RHI/Drawcalls Avg"] = float(str(summary.get("RHI/Drawcalls Avg", 0)).replace(',', ''))
...
def main():
    ...
    files = glob.glob(os.path.join(args.input, "*.html")) if os.path.isdir(args.input) else [args.input]
    ...
    for f in files:
        try:
            ...
            base_name = f"Profile({ts_match.group(1)})" if ts_match else os.path.splitext(os.path.basename(f))[0]
            if f.endswith('_1.html') and not base_name.endswith('_1'): base_name += "_1"
